...
public class MechanicWindow extends JFrame {
    ...
    // Komponenty pre použitie dielu – referencie pre refresh
    private JComboBox<String> usePartOrderBox;
    private JComboBox<StockPartItem> usePartCombo;
    private JTextField usePartQtyF;
    ...
    private void buildUI() {
        ...
        tabs.addTab(" Dodatočné závady",     buildFaultsTab());
        tabs.addTab(" Notifikácie",           notifPanel);
        tabs.addChangeListener(e -> {
            if (tabs.getSelectedIndex() == 0) refreshAssigned();
            if (tabs.getSelectedIndex() == 1) refreshUsePartCombo();
        });
    ...
    private JPanel buildWorkItemsTab() {
        ...
        //  Formulár: pridať pracovnú položku 
 ...
 " Nová požiadavka [" + req.getRequestId() + "]: "
        ...
        //  Formulár: POUŽIŤ diel zo skladu 
        usePartCombo = new JComboBox<>();
        usePartQtyF  = new JTextField("1", 4);
        refreshUsePartCombo();

        JButton usePartBtn    = UIUtils.successButton(" Použiť diel na zákazku");
        JButton refreshStockBtn = UIUtils.outlineButton("Obnoviť sklad");

        usePartBtn.addActionListener(e -> doUsePart(orderBox));
        refreshStockBtn.addActionListener(e -> refreshUsePartCombo());
        ...
        JPanel usePartForm = new JPanel(new FlowLayout(FlowLayout.LEFT));
        usePartForm.setBorder(BorderFactory.createTitledBorder("Použiť diel zo skladu na zákazku"));
        usePartCombo.setPreferredSize(new Dimension(280, 26));
        usePartForm.add(new JLabel("Diel:"));  usePartForm.add(usePartCombo);
        usePartForm.add(new JLabel("Qty:"));   usePartForm.add(usePartQtyF);
        usePartForm.add(usePartBtn);
        usePartForm.add(refreshStockBtn);
        ...
        JPanel forms = new JPanel(new GridLayout(3, 1));
        forms.add(wiForm);
        forms.add(reqForm);
        forms.add(usePartForm);
    ...
    /**
     * Naplní combobox dostupnými dielmi v sklade.
     * Prioritne zobrazí diely objednané pre aktívnu zákazku.
     */
    private void refreshUsePartCombo() {
        if (usePartCombo == null) return;
        usePartCombo.removeAllItems();

        // Zisti aktuálne vybranú zákazku
        String currentOrderId = null;
        if (usePartOrderBox != null && usePartOrderBox.getSelectedIndex() >= 0) {
            List<Order> orders = orderService.getOrdersForMechanic(String.valueOf(employee.getEmployeeId()));
            int idx = usePartOrderBox.getSelectedIndex();
            if (idx < orders.size()) currentOrderId = orders.get(idx).getOrderId();
        }

        List<SparePart> available = currentOrderId != null
                ? inventory.getAvailablePartsForOrder(currentOrderId)
                : inventory.getAvailableParts();

        for (SparePart sp : available) {
            usePartCombo.addItem(new StockPartItem(sp, inventory.getStock(sp.getPartId())));
        }

        if (usePartCombo.getItemCount() == 0) {
            usePartCombo.setEnabled(false);
        } else {
            usePartCombo.setEnabled(true);
        }
    }

    /**
     * Mechanik použije vybraný diel zo skladu:
     * 1. Odpočíta zásoby v sklade
     * 2. Pridá SparePart na zákazku
     * 3. Prepočíta celkovú cenu zákazky
     */
    private void doUsePart(JComboBox<String> orderBox) {
        Order o = resolveSelectedOrder(orderBox);
        if (o == null) { UIUtils.showError(this, "Vyberte zákazku."); return; }

        StockPartItem selected = (StockPartItem) usePartCombo.getSelectedItem();
        if (selected == null) { UIUtils.showError(this, "Žiadne diely na sklade."); return; }

        int qty = 1;
        try { qty = Integer.parseInt(usePartQtyF.getText().trim()); }
        catch (NumberFormatException ex) { UIUtils.showError(this, "Neplatné množstvo."); return; }
        if (qty <= 0) { UIUtils.showError(this, "Množstvo musí byť aspoň 1."); return; }

        SparePart sp = selected.getSparePart();
        int available = inventory.getStock(sp.getPartId());
        if (available < qty) {
            UIUtils.showError(this, "Nedostatok zásob. Na sklade: " + available + " ks.");
            return;
        }

        // Odpočítať zo skladu
        boolean consumed = inventory.consumePart(sp.getPartId(), qty);
        if (!consumed) {
            UIUtils.showError(this, "Nepodarilo sa odpočítať diel zo skladu.");
            return;
        }

        // Pridať diel na zákazku (s aktuálnym množstvom)
        SparePart usedPart = new SparePart(sp.getPartId(), sp.getName(), sp.getUnitPrice(), String.valueOf(qty));
        o.addSparePart(usedPart);
        o.calculateCost(o.getWorkItems(), o.getSpareParts());
        ctx.getOrderRepository().save(o);
 ...
 "Diel \"" + sp.getName() + "\" (qty: " + qty + ") bol použitý na zákazku.\n"
                        + "Zostatok na sklade: " + inventory.getStock(sp.getPartId()) + " ks.\n"
                        + "Cena zákazky aktualizovaná na: " + String.format("%.2f €", o.getTotalCost()));
    }
    ...
    private JPanel buildFaultsTab() {
        ...
        JButton reportBtn = UIUtils.dangerButton(" Nahlásiť závadu zákazníkovi (čaká na schválenie)");
 ...
 "Mechanik zistil dodatočnú závadu na Vašom vozidle: " + desc
    ...
    // =========================================================
    // Pomocná trieda pre položky v comboboxe skladu
    // =========================================================
    private static class StockPartItem {
        private final SparePart sparePart;
        private final int stockQty;

        StockPartItem(SparePart sp, int stockQty) {
            this.sparePart = sp;
        ...
        public String toString() {
            return sparePart.getName()
                    + " [" + sparePart.getPartId() + "]"
                    + "  – " + String.format("%.2f €", sparePart.getUnitPrice())
                    + "  (sklad: " + stockQty + " ks)";
        }
    }
}